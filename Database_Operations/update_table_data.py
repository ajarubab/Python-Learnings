...
try:
    ...
    mycursor.execute("SELECT COUNT(*) FROM employeeInfo WHERE Emp_name='Ramesh'")
    count = mycursor.fetchone()[0]  # Fetch the count of rows

    if count > 0:
        # If the record is found, perform the update
        mycursor.execute("UPDATE employeeInfo SET Emp_name='Rajesh' WHERE Emp_name='Ramesh'")
        con.commit()
        ...
        print("\n\nUpdate not possible as data not found in table.")

...
try:
    ...
    mycursor.execute("SELECT COUNT(*) FROM employeeInfo WHERE Emp_name='Amar'")
    count = mycursor.fetchone()[0]  # Fetch the count of rows

    if count > 0:
        # If the record is found, perform the delete
        mycursor.execute("DELETE FROM employeeInfo WHERE Emp_name='Amar'")
        con.commit()
        print("\nBasic data deletion done successfully.")
